...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // add event listener
    window.addEventListener('scroll', hahndleScroll);

    // clear event listener
    return () => {
      window.removeEventListener('scroll', hahndleScroll);
    }
  },[])

  return (
    <header 
      className={`fixed z-50 w-full transition-all ${active ? 'bg-[#030315] py-6' : 'bg-transparent py-8'}`}
    >
